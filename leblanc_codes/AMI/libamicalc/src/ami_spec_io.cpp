...
std::cout<<std::endl;
print_reg_prod_t(term.regprod_);
std::cout<<std::endl;
...
std::cout<<"Aterm "<<i<<" with effective stat "<<Aprod[i].eff_stat_<<std::endl;	
...
void AmiSpec::print_reg_prod_t(delta_prod_t &regprod){
	
std::cout<<"--- Regulator PRODUCT - SIZE="<< regprod.size()<<"---"<<std::endl;	
for(int i=0; i< regprod.size(); i++){

std::cout<<i<<" ";
print_delta_t(regprod[i]);
