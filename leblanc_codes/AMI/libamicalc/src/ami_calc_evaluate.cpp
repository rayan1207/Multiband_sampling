...
// if overflow is detected the result needs to be discarded so just return immediately. 
if( amibase.overflow_detected){
overflow_detected=true;	
std::cout<<std::setprecision(20)<<"Exiting due to overflow "<< calc_result<<std::endl;
return;
}
...
// TODO: This function is incomplete - depricated 
...
int size=state.internal_freq_size_;//internal_k_list_.size();
...
// MAR. 30 2022 - this assumed that starting epsilons were always +1. think this needs to be updated
// Double check something 
result[j]=eval_epsilon(state.t_list_[j], state.tp_list_[j], construct_k(R0[i].alpha_ , k_list) , R0[i].species_, external.MU_, external.H_, state.disp_)*double(R0[i].eps_[j]);
}else{
