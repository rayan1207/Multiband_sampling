...
std::ofstream file;
std::string fname="sto6g_james_"+std::to_string(max)+"_beta"+std::to_string(beta)+"_ext"+std::to_string(ext_band)+"_bc"+std::to_string(bands)+".dat";
...
for(int n=nmin; n<nmax; n++){
    ...
    double gamma=1e-3;//1e-2;//M_PI/beta*(2*n+1);//1e-4;//M_PI/beta*(2*n+1);//1e-3;//M_PI/beta*(2*n+1);//1e-3;	
...
ext.external_freq_[0]=std::complex<double>(freq,gamma);// M_PI/ext.BETA_*(2*n+1));			//std::complex<double>(0.0,freq);//M_PI/ext.BETA_);
