...
AmiBase::construct(ami_parms &parms, g_prod_t R0, R_t &R_array,
  ...
  // Explanation: for each GProd in R, for each pole in that Gprod, find
  // residue for each and collect those in an array.
 ...
 * @param[in] array_index: indicator for which element of `alpha_t` corresponds
 * to that integration variable. Typically the same as int_index, unless you
 * want to perform out of order.
 ...
 *
 */
void AmiBase::update_gprod_general(int int_index, int array_index,
                                   AmiBase::R_t &R_array, AmiBase::P_t &P_array,
                                   AmiBase::S_t &S_array) {
  Ri_t g_in;
  ...
  for (int j = 0; j < R_array[array_index].size(); j++) {
    ...
    for (int i = 0; i < poles.size(); i++) {
      if (poles[i].multiplicity_ == 1) {
        ...
        s_out.push_back(
            get_simple_sign(int_index, R_array[array_index][j], poles[i]));

        p_out.push_back(poles[i]);
      } else {
        Ri_t temp_g;

        evaluate_general_residue(R_array[array_index][j], poles[i], temp_g,
                                 temp_p, temp_s);
 ...
 */
AmiBase::g_prod_t AmiBase::simple_residue(AmiBase::g_prod_t G_in,
 ...
 *
 */
AmiBase::g_struct AmiBase::update_G_pole(AmiBase::g_struct g_in,
                                         AmiBase::pole_struct pole) {
 ...
 *
 */
AmiBase::pole_struct AmiBase::update_Z_pole(AmiBase::pole_struct p_in,
                                            AmiBase::pole_struct pole) {
  AmiBase::pole_struct p_new;

  for (int i = 0; i < p_in.alpha_.size(); i++) {
    if (i != pole.index_) {
      p_new.alpha_.push_back(p_in.alpha_[i] +
                             p_in.alpha_[pole.index_] * pole.alpha_[i]);
    } else {
  ...
  for (int i = 0; i < p_in.eps_.size(); i++) {
    p_new.eps_.push_back(p_in.eps_[i] +
                         p_in.alpha_[pole.index_] * pole.eps_[i]);
  }

  if (p_new.alpha_.size() != p_in.alpha_.size()) {
    std::cerr << "Error: Something may be wrong. Alphas of G and pole not the "
                 "same size"
              << std::endl;
  }
...
/**
 * Depricated version of update_gprod for only simple poles. Replaced by
 * update_gprod_general.
 */
void AmiBase::update_gprod_simple(int index, AmiBase::R_t &R_array,
                                  AmiBase::P_t &P_array,
                                  AmiBase::S_t &S_array) {
  int next = index + 1;
...
/**
 *
 * Void function takes a product of Green's functions - an element of an `Ri_t`
 * object - and obtains the residue for a specific pole. Output is an array of
 * such elements, a full `Ri_T` object, along with the respective poles and
 * signs for `Pi_t` and  `Si_t`. Includes derivative.
 ...
 */
void AmiBase::evaluate_general_residue(AmiBase::g_prod_t G_in,
                                       AmiBase::pole_struct pole,
                                       AmiBase::Ri_t &Ri_out,
 ...
 * This is accomplished via multiple applications of chain rule.  This is
 * described in PRB 99 035120 - though the notation changed somewhat.  The
 * prefactor of -1 is no longer absorbed into a green's function, but instead
 * is stored in the `S_t` array.  What is omitted there is discussion of
 * derivatives of the Fermi functions.  Here they are simply tracked by
 * defining an integer `der_` of the `pole-struct` that gets incremented.
 *
 */
void AmiBase::take_derivative_gprod(AmiBase::g_prod_t &g_prod,
                                    AmiBase::pole_struct pole,
                                    double start_sign, AmiBase::Ri_t &r_out,
                                    AmiBase::pole_array_t &poles,
                                    AmiBase::sign_t &signs) {
  poles.clear();
  signs.clear();
 ...
 * multiplicity!=1.
 */
double AmiBase::get_starting_sign(AmiBase::g_prod_t G_in,
                                  AmiBase::pole_struct pole) {
  double result = 1.0;
 ...
 * residue (z-z0)^m
 */
AmiBase::g_prod_t AmiBase::reduce_gprod(AmiBase::g_prod_t G_in,
                                        AmiBase::pole_struct pole) {
  g_prod_t reduced;
