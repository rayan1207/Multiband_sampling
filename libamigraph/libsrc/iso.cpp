...
main()
{
  ...
  for (std::size_t v = 0; v != f.size(); ++v)
    std::cout << get(v2_index_map, f[v]) << " ";
  std::cout << std::endl;
  
  for (std::size_t v = 0; v != f.size(); ++v)
    std::cout << get(v1_index_map, f[v]) << " ";
  std::cout << std::endl;
  
  std::cout<<get(v1_index_map,v1[0])<<std::endl;
  std::cout<<get(v1_index_map,v1[3])<<std::endl;
  
  std::cout<<v1_index_map(f[0])<<" "<< v2_index_map(f[0])<<std::endl;
  // std::cout<<g1(f[0])<<std::endl;
  // std::cout<<g1[f[0]].index_<<std::endl;
  
  
  // for (boost::tie(i, end) = vertices(g1); i != end; ++i, ++id){
	  // std::cout<< v1_index_map[*i]<<" ";
  // }
  // std::cout<<std::endl;
  
 // for (std::size_t v = 0; v != f.size(); ++v){
	// for (boost::tie(i, end) = vertices(g1); i != end; ++i, ++id){ 
	// if( v1_index_map[*i]==get(get(vertex_index, g2), f[v])){
    // std::cout << "v1 index: "<< v1_index_map[*i]<<" is equal to "<<get(get(vertex_index, g2), f[v]) << " "<<std::endl;;
	// }
	// }
 // }
